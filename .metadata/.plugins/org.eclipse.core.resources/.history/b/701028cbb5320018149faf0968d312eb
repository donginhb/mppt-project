...
static void MX_TIM1_Init(void)
{
	  ...
	  sBreakDeadTimeConfig.DeadTime =  8; //100;
					...
					while (isCharging)
	  				{
	  					...
	  					if (getADC == 1)
	  		  			...
	  		  			// monitor solar array voltage and current
	  		  			if (iSolarArray <= THRESHOLD_CURRENT)
	  		  			{
	  		  				switchSolarArray(OFF);
	  		  				isCharging = 0;
	  		  			...
	  		  			if (vBattery >= FloatVoltage(tempAmbient) || (warning == HIBATTV) )
	  		  			{
	  		  				switchSolarArray(OFF);
	  		  				isCharging = 0;
	  		  				canCharge = 0;
	  		  			...
	  		  			// This is here to prevent catastrophic failure!
	  		  			// If vSolarArray gets too high, we risk damaging PCB components
	  		  			// on the solar array (Vin) side of the bridge. As as result, we also risk frying
	  		  			// the processor and the 3.3v supply.

	  		  			// Given the IV characteristics of a "typical" solar panel used with this charger, there
	  		  			// is no reason for this voltage to exceed 20v.
	  		  			// This circuit can generate voltages across the solar array in excess of 30v!
	  		  			// ( The relationship is Duty Cycle = V(battery) / V(solar array) )
